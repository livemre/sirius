...
uint256 hashGenesisBlock("0x64fd2fa4d98f81c256729cbba901395de638073b26527e3c6eeaca50498b916c");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sirius: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sirius Signed Message:\n";
...
{
    // Sirius: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Sirius: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Sirius: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Sirius: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sirius: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sirius: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7a276b4b6f23e7d2eb39af137eb3066d90583d642d23a34d34a7ba43709319da");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1559092430;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087113795;

        if (fTestNet)
        {
            block.nTime    = 1559092353;
            block.nNonce   = 386081229;
        }
...
#if defined(M_X64) || defined(x86_64) || defined(_M_AMD64) || (defined(MAC_OSX) && defined(i386_))
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Sirius: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SiriusMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SiriusMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SiriusMiner(CWallet *pwallet)
{
    printf("SiriusMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sirius-miner");
    ...
    try { loop {
        ...
        printf("Running SiriusMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SiriusMiner, pwallet));
}
