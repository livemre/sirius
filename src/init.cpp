...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Sirius version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  siriusd [options]                     " + "\n" +
                  "  siriusd [options] <command> [params]  " + _("Send command to -server or siriusd") + "\n" +
                  "  siriusd [options] help                " + _("List commands") + "\n" +
                  "  siriusd [options] help <command>      " + _("Get help for a command") + "\n";
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: sirius.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: siriusd.pid)") + "\n" +
        ...
        "\n" + _("SSL options: (see the Sirius Wiki for SSL setup instructions)") + "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Sirius is probably already running."), strDataDir.c_str()));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    printf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    printf("Sirius version %s (%s)\n", FormatFullVersion().c_str(), CLIENT_DATE.c_str());
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Sirius") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Sirius to complete") << "\n";
